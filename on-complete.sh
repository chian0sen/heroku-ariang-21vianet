...
#export RCLONE_CACHE_CHUNK_SIZE=6.25M
...
UPLOAD_FILE() {
    ...
    while [ ${RETRY} -le ${RETRY_NUM} ]; do
        ...
        rclone copy -v "${UPLOAD_PATH}" "${REMOTE_PATH}" --cache-chunk-size 1M --transfers=1
        RCLONE_EXIT_CODE=$?
		RCLONE_EXIT_CODE_2=0
		if [ -n "${RCLONE_DESTINATION_2}" ]; then
			rclone copy -v "${UPLOAD_PATH}" "${REMOTE_PATH_2}" --cache-chunk-size 1M --transfers=1
			RCLONE_EXIT_CODE_2=$?
		fi
